...
export function getOverallAnalysis(sslResult, vtResult) {
    ...
    const vtDangerous = vtResult.includes("High Risk") || vtResult.includes("Dangerous");
    ...
    if (vtDangerous) {
        symbol = "🚨";
        verdict = "DANGEROUS";
        recommendation = "Do not visit this site. Multiple security vendors have flagged it as malicious.";
    } else if (vtSuspicious && sslWarning) {
        symbol = "❌";
        verdict = "HIGH RISK";
        recommendation = "Avoid this site. Both SSL configuration and threat analysis show security concerns.";
    } else if (vtSuspicious) {
        symbol = "⚠️";
        verdict = "MEDIUM RISK";
        recommendation = "Exercise caution. Some security vendors have flagged this domain.";
    } else if (sslWarning && vtClean) {
        symbol = "⚠️";
        verdict = "MEDIUM RISK";
        recommendation = "SSL certificate issues detected, but no malicious content found. Proceed with caution.";
        ...
        symbol = "⚠️";
        verdict = "UNKNOWN";
        recommendation = "SSL certificate is valid, but threat analysis was unavailable. Try running again later or use caution when visiting the website.";
    } else if (sslUnknown && vtClean) {
        symbol = "⚠️";
        verdict = "LIKELY SAFE";
        recommendation = "No security threats detected, though SSL analysis was inconclusive.";
    } else {
        symbol = "⚠️";
        verdict = "UNKNOWN";
